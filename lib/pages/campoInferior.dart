...
class CampoInf extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    secondPath.addRect(Rect.fromLTWH((size.width/8) , size.height - 200, (size.width) - (size.width/4), size.height));
    path.addPath(secondPath, Offset(0.0, 0.0));

    Path thirdPath = Path();
    thirdPath.addRect(
        Rect.fromLTWH((size.width/4), size.height - 150, (size.width / 2), size.height));
    ...
    Path fifthPath = Path();
    path.addArc(
        Rect.fromLTWH((size.width / 2) - ((size.width / 4) / 2), size.height - (150 + (size.width / 4)),
